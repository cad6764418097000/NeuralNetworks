const LOG_ON = true; // whether or not to error console.log
...
class NeuralNetwork {
    ...
    feedForward(inputArray) {
        ...
        this.hidden = Matrix.add(this.hidden, this.bias0);   // add bias
        ...
        outputs = Matrix.add(outputs, this.bias1);    // add bias
    ...
    train(inputArray, targetArray) {
        ...
        if(LOG_ON){
          if(this.logCount == LOG_FREQ){
            console.log("Errors = O1: " + outputErrors.data[0][0]);
          }
          this.logCount -= 1;
          if (this.logCount == 0) {
        ...
        // calculate the hidden deltas (erroirs * derivitive of hidden)
        let hiddenDerivs = Matrix.map(this.hidden, x => sigmoid(x, true));
        let hiddenDeltas = Matrix.multiply(hiddenErrors, hiddenDerivs);

        // update the weightds (add the transpose of the layers "dot" deltas)
